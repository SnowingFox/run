...
2. Reference Setting
   - Run `node .ttadk/scripts/get-stack-context.js json` to load tech stack references. Use these references for task breakdown and implementation guidance.

## Outline

1. **Setup**: Run `node .ttadk/scripts/check-prerequisites.js --json` from repo root and parse FEATURE_DIR and AVAILABLE_DOCS list. All paths must be absolute.
...
4. **Generate tasks.md**: Use `.ttadk/templates/tasks-template.md` as structure, fill with:
