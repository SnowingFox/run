...
async function setupPlan(options = {}) {
    ...
    // Log current branch info (no validation needed - any branch is valid)
    if (!options.json) {
        console.log(`[ttadk] Using branch: ${paths.CURRENT_BRANCH}`);
    }
    ...
    const template = path.join(repoRoot, '.ttadk', 'templates', 'plan-template.md');
