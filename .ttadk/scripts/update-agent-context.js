 ...
 */
function updateAgentFile(agentFilePath, featureBranch, techStack, language, projectType, repoRoot) {
    const currentDate = new Date().toISOString().split('T')[0];
    const projectName = path.basename(repoRoot);
    // Read template
    const templatePath = path.join(repoRoot, '.ttadk', 'templates', 'agent-file-template.md');
...
`;
        ...
        content = content.replace(/\[EXTRACTED FROM ALL PLAN\.MD FILES\]/g, `- ${featureBranch}: ${techStack}`);
        ...
        content = content.replace(/\[LAST 3 FEATURES AND WHAT THEY ADDED\]/g, `- ${featureBranch}: Added ${techStack}`);
        // Ensure directory exists
        const dir = path.dirname(agentFilePath);
        if (!(0, common_1.existsSync)(dir)) {
            fs.mkdirSync(dir, { recursive: true });
        ...
        if (techMatch) {
            ...
            // Check if this feature branch already exists
            if (!existingTech.includes(featureBranch)) {
                const newTech = existingTech.trim() + `\n- ${featureBranch}: ${techStack}`;
                content = content.replace(techRegex, `## Active Technologies\n${newTech}\n\n`);
            }
            else {
                // Update existing entry
                const branchRegex = new RegExp(`- ${featureBranch}:.*`, 'g');
                content = content.replace(branchRegex, `- ${featureBranch}: ${techStack}`);
            }
        }
        ...
        if (changesMatch) {
            ...
            const newChange = `- ${featureBranch}: Added ${techStack}`;
            // Remove existing entry for this branch if exists
            const filteredChanges = existingChanges.filter(change => !change.includes(featureBranch));
...
async function updateAgentContext(agentType = 'claude', options = {}) {
    ...
    // Update agent file
    updateAgentFile(agentFilePath, paths.CURRENT_BRANCH, techStack, language, projectType, repoRoot);
    return {
        AGENT_FILE: agentFilePath,
        FEATURE_BRANCH: paths.CURRENT_BRANCH,
...
`);
    ...
    updateAgentContext(agentType, options)
        ...
        if (options.json) {
            ...
            console.log(`FEATURE_BRANCH: ${result.FEATURE_BRANCH}`);
