 ...
 * - Archives ALL spec directories (including current branch)
 * - Current branch's spec directory is NOT deleted after archiving
...
function updateArchiveIndex(archiveIndexPath, metadata) {
    const header = `# Archived Specs\n\n| Branch | Description | Archived | Size | Path |\n|--------|-------------|----------|------|------|\n`;
    let rows = [];
    ...
    const existingIndex = rows.findIndex(r => r.branch === metadata.branch);
    const newRow = {
    ...
    const tableRows = rows.map(row => `| ${row.branch} | ${row.description} | ${row.archived} | ${row.size} | ${row.path} |`).join('\n');
 ...
 */
function branchToFilename(branch) {
...
async function archiveSpecs(options = {}) {
    ...
    try {
        ...
        // Get current branch
        const currentBranch = (0, common_1.getCurrentBranch)();
        ...
        if (options.dryRun) {
            ...
            for (const spec of allSpecs) {
                const isCurrentBranch = spec.branch === currentBranch;
                const estimatedSize = estimateDirectorySize(spec.fullPath);
                const archiveFilename = `${branchToFilename(spec.branch)}.tar.gz`;
        ...
        for (const spec of allSpecs) {
            ...
            // Delete original folder (except current branch)
            const shouldDelete = !isCurrentBranch;
            if (shouldDelete) {
...
BEHAVIOR:
  ...
  - Archives ALL spec directories (including current branch)
  - Current branch's spec directory is NOT deleted
  - Other spec directories ARE deleted after archiving
  - archived/ and doc_export/ directories are never touched
            ...
            if (result.success) {
                if (result.archived.length === 0) {
                    ...
                    for (const spec of result.archived) {
                        ...
                        console.log(`  ${spec.branch} ${status}`);
