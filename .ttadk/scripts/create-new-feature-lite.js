...
function copyTemplate(repoRoot, templateName, featureDir, targetFileName, fallbackContent) {
    const templatePath = path.join(repoRoot, '.ttadk', 'templates', templateName);
...
async function createNewFeatureLite(_featureDescription, _options = {}) {
...
if (require.main === module) {
    ...
    if (args.includes('--help') || args.includes('-h')) {
        console.log(`Usage: create-new-feature-lite [--json]`);
    ...
    createNewFeatureLite(undefined, options)
        .then(result => {
        if (options.json) {
