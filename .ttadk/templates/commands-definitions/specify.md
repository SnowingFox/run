...
2. Reference Setting
   - Run `node .ttadk/scripts/get-stack-context.js json` to load tech stack references. Use these references for domain-specific requirements and constraints.
...
1. Get current branch and setup feature directory:
- Run `node .ttadk/scripts/create-new-feature.js --json` from repo root.
- This script gets the current branch, creates `specs/{branch}/` directory, and copies spec.md template.
- Parse the JSON output to get FEATURE_DIR and SPEC_FILE path.

2. Load `.ttadk/templates/spec-template.md` to understand required sections.
...
6. Report completion with branch name, spec file path, checklist results, and readiness for the next phase (`/adk:clarify` or `/adk:plan`).

**NOTE:** The script uses the current git branch (or defaults to 'master' if not in a git repo) and initializes the spec file before writing.
